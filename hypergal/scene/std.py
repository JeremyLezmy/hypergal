...
class STD_SliceScene( PointSource, SliceScene):
    ...
    def guess_parameters(self):
        ...
        xoff,yoff = np.median(self.slice_comp.index_to_xy(self.slice_comp.get_brightest_spaxels(5)),axis=0)
        
        geom_guess = dict({'xoff':xoff, 'yoff':yoff})
        
        psf_guess= dict( {k+'_ps':v for k,v in self.psf.guess_parameters().items()})       
        ...
        if self.has_curved_bkgd:
            base_guess.update(**{k:0 for k in BackgroundCurved.BACKGROUND_PARAMETERS if k not in ['background']})
    ...
    def update(self, ignore_extra=False, **kwargs):
        ...
        for k in psfparams.keys():
            if k in self.PSF_PARAMETERS:
                self.psf.update_parameters(**{k.replace('_ps',''):psfparams[k]})
    ...
    def get_model(self, ampl_ps=None, background_ps=None, overlayparam=None, psfparam=None, bkgdx=None, bkgdy=None, bkgdxy=None, bkgdxx=None, bkgdyy=None, fill_comp=False):
        ...
        if background_ps is None and not self.has_curved_bkgd:
            ...
            coeffs = dict({k.replace('_ps',''):v for k,v in self.baseparams.items() if k in BackgroundCurved.BACKGROUND_PARAMETERS or k=='background_ps'})
            background_ps = BackgroundCurved.get_background(x, y, coeffs )

        elif background_ps is not None and self.has_curved_bkgd:
            
            x,y = self.slice_comp_xy
            coeffs = dict({'background':background_ps,'bkgdx':bkgdx, 'bkgdy':bkgdy, 'bkgdxy':bkgdxy, 'bkgdxx':bkgdxx, 'bkgdyy':bkgdyy})
            background_ps = BackgroundCurved.get_background(x, y, coeffs )
            
        if psfparam is not None:           
            psf_psparam =  {k.replace('_ps',''):v for k,v in psfparam.items() if k in self.PSF_PARAMETERS}
            
        else:
            psf_psparam = self.psf.parameters
                
        if overlayparam is not None:
            geom_params = {k:v for k,v in overlayparam.items() if k in self.GEOMETRY_PARAMETERS}
        else:
            geom_params = self.overlayparam
            
        ps_profile = super( STD_SliceScene,self).get_model( xoff=geom_params['xoff'], yoff=geom_params['yoff'], ampl=ampl_ps, bkg=background_ps, **psf_psparam)
    ...
    def get_subslice(self, std_slice):
        ...
        x, y = np.median(std_slice.index_to_xy(std_slice.get_brightest_spaxels(5)),axis=0)
        p = Point(x, y)
        circle = p.buffer(20)
        idx = std_slice.get_spaxels_within_polygon( circle )
        sub_slice = std_slice.get_subslice([i for i in std_slice.indexes if i in idx] )
    ...
    def show(self, savefile=None, titles=True, axes=None, vmin='10',vmax='99', cmap='cividis', cmapproj=None, fill_comp=True, index=None, logscale=True, add_colorbar=True ):
        ...
        flux_model = self.get_model(fill_comp=fill_comp) + self.bkgd_comp/self.norm_comp 
        ...
        res /=  flux_model
        RMS = np.sqrt( (1/len(flux_comp) * np.sum(((flux_comp-flux_model)/flux_model)**2)))*100
        prop = {**prop,**{"vmin":-0.5, "vmax":0.5, "cmap":"coolwarm"}}
        title_res = "Residual (data-model)/model [±50%]" + "\n" + fr" RMSEpe = {np.round(RMS,2)}%"
        self.show_psf( ax=axes[2], adjust=True, flux=res, edgecolor=None, vmin=-0.5, vmax=0.5, cmap="coolwarm")
        ...
        if add_colorbar:
            ...
            if logscale:
                ...
                cmapdat.set_label(r'Flux (normalized)', color="0.5", fontsize="medium",labelpad=10)
            ...
            cmapres.set_label(r'Residual', color="0.5", fontsize="medium",labelpad=10)
    ...
    def show_profile(self, ax=None, logscale=True, savefile=None, radius_spx=15):
        ...
        if self.has_curved_bkgd:
            ...
            x,y = self.slice_comp_xy
            coeffs = dict({k.replace('_ps',''):v for k,v in self.baseparams.items() if k in BackgroundCurved.BACKGROUND_PARAMETERS and k!='background'})
        ...
        dxsn = xslice-xcsn
        dysn = yslice-ycsn
        rsn = np.sqrt( dxsn**2 + self.psf.a_ell*dysn**2 + 2*self.psf.b_ell * (dxsn*dysn) )

        beta = self.psf.get_beta()
        x,y = np.meshgrid(np.linspace(0.,radius_spx,100),np.linspace(0.,radius_spx,100))
        xc,yc=0,0
        dx = x-xc
        dy = y-yc
        ...
        ax.plot( radius, (profil*ampl_ps*norm_comp + bkgd*norm_comp +bkgd_comp)/np.max(sndat), label="Gaussian + Moffat model", c='r')
        ax.scatter( rsn, sndat/np.max(sndat), c='k', label="Datas", s=16)
        ax.errorbar(rsn, sndat/np.max(sndat),snerr/np.max(sndat), fmt='none' ,c='k')
        ax.plot(radius, (eta*gaussian*ampl_ps*norm_comp + (bkgd*norm_comp +bkgd_comp)*normalisation)/(np.max(sndat)*normalisation), c='g', label='Gaussian')
        ax.plot(radius, (moffat*ampl_ps*norm_comp+ (bkgd*norm_comp +bkgd_comp)*normalisation)/(np.max(sndat)*normalisation), c='b', label='Moffat')
        ...
        ax.hlines((bkgd*norm_comp +bkgd_comp)/np.max(sndat), 0, radius_spx, ls='--', color='grey', alpha=0.5, label='Background')       
    ...
    def show_contour(self, ax=None, logscale=True, savefile=None, cmap='viridis', vmin='10', vmax='99.9'):
        ...
        if logscale:
            ...
            self.show_psf( ax=ax, adjust=True, flux=np.log(flux_comp), cmap=cmap, vmin=vmin, vmax=vmax, edgecolor=None)

        else:
            vmin, vmax = tools.parse_vmin_vmax(flux_comp, vmin,vmax)
            self.show_psf( ax=ax, adjust=True, flux=flux_comp, cmap=cmap, vmin=vmin, vmax=vmax, edgecolor=None)

        ax.contour(xt,yt, profil*ampl_ps, levels=np.logspace(np.log(np.percentile(flux_comp,96)),np.log(np.max(flux_comp)),10), linestyles='dashed', cmap="YlGn", alpha=0.6)
        sedm=ax.tricontour(np.ravel(xslice.tolist()),np.ravel(yslice.tolist()),np.ravel((flux_comp).tolist()), levels=np.logspace(np.log(np.percentile(flux_comp,96)),np.log(np.max(flux_comp)),10),  cmap="YlGn", alpha=0.6)
    ...
    def show_profile_contour(self, logscale_profile=True, logscale_contour=True,radius_spx=15, cmap='viridis', vmin='10', vmax='99.9', savefile=None):
        ...
        self.show_profile(ax=axp, logscale=logscale_profile, radius_spx=radius_spx )
        self.show_contour(ax=axc, logscale=logscale_contour, cmap=cmap, vmin=vmin, vmax=vmax)
        if savefile is not None:
            fig.savefig(savefile)
    ...
    def report_std(self, logscale_profile=True, logscale_contour=True,radius_spx=15, vmin='10', vmax='99.9',
        ...
        self.show(axes = [ax,axm,axr], logscale=logscale_imshow,  cmap=cmap, vmin=vmin, vmax=vmax, fill_comp=fill_comp, index=index, cmapproj=cmapproj, add_colorbar=add_colorbar, titles=titles )
        self.show_profile(ax=axp, logscale=logscale_profile, radius_spx=radius_spx )
        self.show_contour(ax=axc, logscale=logscale_contour, cmap=cmap, vmin=vmin, vmax=vmax)
    ...
    def fwhm(self):
        ...
        fwhm = radius[np.where(abs(profil/np.max(profil) -0.5) == np.min(abs(profil/np.max(profil) - 0.5)))[0]][0]*2*0.558
    ...
    def overlayparam(self):
        if not hasattr(self, "_overlayparam"):
            self._overlayparam = {k:0. if "xoff" in k else 0. for k in self.GEOMETRY_PARAMETERS}
