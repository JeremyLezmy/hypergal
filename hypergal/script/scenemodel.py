...
class DaskScene( DaskHyperGal ):
    ...
    def compute_single(self, cubefile, radec, redshift,
        ...
        if split:
            ...
            stored.append( self.global_report(calcube, hostmodel, snmodel, bkgdmodel, source_coutcube, self.get_sourcedf(radec, cubefile), bestfit_completfit, bestfit_mfit, radec, redshift, cubefile, lbda_range, nslices, saveplot=saveplot_report) )
    ...
    def global_report( datacub, hostmod, snmod, bkgdmod, coutcube, df,
        ...
        fullmod = datacub.get_new( newdata = hostmod.data + snmod.data + bkgdmod.data)
        ...
        mslices = sniso.to_metaslices(lbda_ranges=mainlbdarange, nslices=nslices, as_slice=True)
        mslicesmod = snmod.to_metaslices(lbda_ranges=mainlbdarange, nslices=nslices, as_slice=True)
        mslicesres = fullres.to_metaslices(lbda_ranges=mainlbdarange, nslices=nslices, as_slice=True)
        ...
        xgrid,ygrid = np.meshgrid(np.linspace(0.2,10,100),np.linspace(0.2,10,100))
        xc,yc=0,0
        dx = xgrid-xc
        dy = ygrid-yc
        ...
        ADRFitter = spectroadr.ADRFitter(xpos=x0,ypos=y0, xpos_err=x0err,ypos_err=y0err, lbda=lbda, init_adr=spectroadr.ADR.from_header(datacub.header))
        ...
        slicepull = pyifu.spectroscopy.Slice.from_data( data =(slicedat - slicemodel)/(np.sqrt(2)*(slice_err)), spaxel_mapping = datacube.spaxel_mapping, spaxel_vertices = datacube.spaxel_vertices, lbda = np.mean(datacube.lbda[lbdacond]))  ##PULL
        ...
        datacube._display_im_(axim=axdat, vmin=vmin, vmax=vmax , lbdalim=mainlbdarange, cmap=cmap,rasterized=False);
        axdat.scatter( *ADRFitter.refract( ADRFitter.fitted_xref, ADRFitter.fitted_yref, 6000), marker='x',color='r', s=32, zorder=10, label='Target')
        axdat.scatter(*hostiso.wcs.all_world2pix(coutcube.wcs.all_pix2world(np.array([df.x[0],df.y[0]])[:,None].T,0),0)[0], marker='x',color='k', s=32, zorder=10, label='Host')
        axdat.legend(loc='lower left')

        modelcube._display_im_(axim=axmod, vmin=vmin, vmax=vmax, lbdalim=mainlbdarange, cmap=cmap,rasterized=False);
        axmod.scatter( *ADRFitter.refract( ADRFitter.fitted_xref, ADRFitter.fitted_yref, 6000), marker='x',color='r',s=32, zorder=10)
        axmod.scatter(*hostiso.wcs.all_world2pix(coutcube.wcs.all_pix2world(np.array([df.x[0],df.y[0]])[:,None].T,0),0)[0], marker='x',color='k', s=32, zorder=10)

        slicepull.show(ax=axpull, show_colorbar=False,vmin=-6,vmax=6, cmap=cmapres,rasterized=False); ##PULL
        ...
        cbar.set_label(r'Flux (erg unit)', color="0.5", fontsize="medium",labelpad=10)
        ...
        axhostisospec.plot(hostmodonly.lbda[flagin], np.nanmean(hostmodonly.data[flagin].T,axis=0), c='r', label='Host Model')
        axhostisospec.plot(hostobsonly.lbda[flagin], np.nanmean(hostobsonly.data[flagin].T,axis=0), label='Obs. Host isolated')
        ...
        idx = [(np.abs(hostobsonly.lbda[flagin]-xl)).argmin() for xl in xemlines_gal]
        yemlines_gal = np.nanmean(hostobsonly.data[flagin].T,axis=0)[idx]

        xo3lines_gal = o3_lines(redshift)
        idx = [(np.abs(hostobsonly.lbda[flagin]-xl)).argmin() for xl in xo3lines_gal]
        ...
        idx = [(np.abs(hostobsonly.lbda[flagin]-xl)).argmin() for xl in xablines_gal]
        yablines_gal = np.nanmean(hostobsonly.data[flagin].T,axis=0)[idx]

        axhostisospec.vlines( em_lines(redshift), ymin=yemlines_gal + 0.1*np.median(np.nanmean(hostmodonly.data[flagin].T,axis=0)), ymax=yemlines_gal + 0.15*np.median(np.nanmean(hostmodonly.data[flagin].T,axis=0)), color='k', alpha=0.5, ls='--', label='EM/AB lines' + '\n' + f'Input z={np.round(redshift,4)}')
        axhostisospec.vlines( ab_lines(redshift), ymin=yablines_gal - 0.1*np.median(np.nanmean(hostmodonly.data[flagin].T,axis=0)), ymax=yablines_gal - 0.15*np.median(np.nanmean(hostmodonly.data[flagin].T,axis=0)), color='k', alpha=0.5, ls='--')
        axhostisospec.vlines( xo3lines_gal, ymin=yo3lines_gal + 0.1*np.median(np.nanmean(hostmodonly.data[flagin].T,axis=0)), ymax=yo3lines_gal + 0.15*np.median(np.nanmean(hostmodonly.data[flagin].T,axis=0)), color='k', alpha=0.5, ls='--')
        for l in range(len(all_em_names)):
            axhostisospec.text(em_lines(redshift)[l], yemlines_gal[l] + 0.17*np.median(np.nanmean(hostmodonly.data[flagin].T,axis=0)) , all_em_names[l], ha='center', va='center' )

        axhostisospec.text(xo3lines_gal, yo3lines_gal + 0.17*np.median(np.nanmean(hostmodonly.data[flagin].T,axis=0)) , r'$O[III]$', ha='center', va='center' )

        for l in range(len(all_ab_names)):
            axhostisospec.text(ab_lines(redshift)[l], yablines_gal[l] - 0.17*np.median(np.nanmean(hostmodonly.data[flagin].T,axis=0)) , all_ab_names[l], ha='center', va='center' )
        ...
        sniso.get_partial_cube(idx, np.arange(len(sniso.lbda)))._display_im_(axim=axsnisozoom, vmax='95', vmin='10' , lbdalim=mainlbdarange,rasterized=False);
        axsnisozoom.set_aspect('equal')
        axsnisozoom.set_title('SN isolated', **prop)
        axsnisozoom.set_axis_off()

        snmod.get_partial_cube(idx, np.arange(len(snmod.lbda)))._display_im_(axim=axsnmodzoom, vmax='95', vmin='10' , lbdalim=mainlbdarange,rasterized=False);
        ...
        sl_subpull.show(ax=axsnreszoom, cmap=cmapres, vmin=-6, vmax=6, show_colorbar=False); 
        ...
        axsnprof.plot( radiusrav, profil*ampl_ps*norm_comp/np.max(sndat), label="Gaussian + Moffat model", c='r')
        axsnprof.scatter( rsn, sndat/np.max(sndat), c='k', label="Datas", s=16)
        axsnprof.errorbar(rsn, sndat/np.max(sndat),snerr/np.max(sndat), fmt='none' ,c='k')
        ...
        axsnprof.set_title(fr'SN profile with Gaussian + Moffat model (from Metaslice at {np.round(mslice.lbda,0)} $\AA$)', **prop)
        ...
        axsnspec.fill_between(speclbda, specval+specerr, specval-specerr, alpha=0.3)
        axsnspec.set_xlim(3800,9300)
        axsnspec.set_ylim(0, np.max(specval[(speclbda>3800) & (speclbda<9300)]))
        ...
        l1t, l1b = gs1bl.get_grid_positions(fig)[0]+0.01, gs1bl.get_grid_positions(fig)[1]-0.01
        ...
        lgt, lgb = gs0.get_grid_positions(fig)[1]+0.05, gs0.get_grid_positions(fig)[1]+0.03
        ...
        linesep = plt.Line2D((.5,.5),(lgb,gs2.get_grid_positions(fig)[0]), color="k", linewidth=1.5)
        fig.add_artist(linesep)

        import datetime
        import hypergal
        fig.text( 0.5,0.01,f"hypergal version {hypergal.__version__} | made the {datetime.datetime.now().date().isoformat()} | J.Lezmy (lezmy@ipnl.in2p3.fr)", ha='center', color='grey', fontsize=10)
        fig.suptitle(datacube.header['NAME'] + fr' ({datacube.header["OBSDATE"]} , ID: {datacube.header["OBSTIME"].rsplit(".")[0].replace(":","-")})' , fontsize=16, fontweight="bold",y=0.97)
