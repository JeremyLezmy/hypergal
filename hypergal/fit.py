...
class FitSlices( object ):

    def __init__(self, slice_in, slice_comp, xy_in=None, xy_comp=None, load_overlay=True, **kwargs):
        """ """
    ...
    @classmethod
    def from_slices(cls, slice_in, slice_comp, xy_in=None, xy_comp=None, **kwargs):
        """ """
        return cls(slice_in, slice_comp, xy_in=xy_in, xy_comp=xy_comp, **kwargs)
    ...
    def get_modeldata(self, xy_offset=None, scale=None, rotation=None,
        ...
        flux_in, variance_in = self.get_convolved_data_in(psfparam)
    ...
    def get_convolved_flux_in(self, psfconv):
        """ """
        if psfconv is None:
            return self.flux_in, self.variance_in

        print("JEREMY, APPLY HERE CONVOLUTION OF SLICE_IN")
        # THEN RENORM THE CONVOLVED FLUX BY NORM_IN (variance hy norm_in**2)
        # THE RETURNED THE CONVOLVED (and normed) FLUX.
